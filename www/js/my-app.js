...
function googlemapinitialize(mapdiv1,map1,mapsearchInput1,val0,textcaption,thisid,saveto,type) {
	...
	document.getElementById(mapdiv1).style.display="none";
	return false;	
	}
   var latlng = new google.maps.LatLng(7,80);
   ...
   if(!document.getElementById(mapsearchInput1)) {
	
	const controlText = document.createElement("input");	 
	controlText.type = "text";
	controlText.id = mapsearchInput1;  
	controlText.classList.add('input-controls');
	controlText.classList.add('searchInput');
	
	
	document.getElementById(mapdiv1).appendChild(controlText);
	
   }
   
  
    var input = document.getElementById(mapsearchInput1);
    ...
    autocomplete.addListener('place_changed', function() {alert('dd');
        infowindow.close();
        ...
        bindDataToForm(place.formatted_address,place.geometry.location.lat(),place.geometry.location.lng(),val0,textcaption,thisid,saveto,type);
    ...
    google.maps.event.addListener(marker, 'dragend', function() {
        ...
        if (status == google.maps.GeocoderStatus.OK) {
          if (results[0]) {        
              bindDataToForm(results[0].formatted_address,marker.getPosition().lat(),marker.getPosition().lng(),val0,textcaption,thisid,saveto,type);
...
function bindDataToForm(address,lat,lng,val0,textcaption,thisid,saveto,type){
   ...
   if(type == "TransportRequest" & saveto == "pending_cases.info1")
   document.getElementById(val0).value = address + '#mulupdinfo10##' + lng + '**info11##' +lat;

   if(type == "TransportRequest" & saveto == "pending_cases.info2")
   document.getElementById(val0).value = address + '#mulupdinfo12##' + lng + '**info13##' +lat;

   updatetabelonchange(textcaption,thisid,document.getElementById(val0).value,saveto,type);
   document.getElementById(val0).value = address;
}

function googlemapinitializehide(mapdiv1,map1,mapsearchInput1,val0) {
	document.getElementById(mapdiv1).style.display="none";
